...
import { camelToKebab, kebabToCamel, normalizeArgs, setNestedValue } from '../src/args.ts'
import {
  buildCompletionTree,
...
import { getExecutionEnvironment, inferShell, resolveEnvVars, toEnvPrefix } from '../src/env.ts'
...
describe('Nested JSON Schema args', () => {
  ...
  test('jsonSchemaToCittyArgs stringifies non-boolean defaults for citty compatibility', () => {
    const { jsonSchemaToCittyArgs } = require('../src/args.ts') as typeof import('../src/args.ts')
    const schema: JsonSchema = {
      type: 'object',
      properties: {
    ...
    expect(args['replicas'].default).toBe('2')
    expect(args['dry-run'].default).toBe(false)
  })
})
